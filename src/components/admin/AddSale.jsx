 ...
 const uploadProductDataToFirebase = async (e, newProductdata) => {
   ...
   try {
     const videoUrls = await Promise.all(
       newProductdata.videos.map(async (product, index) => {
         const videoRef = ref(storage, `Videos/${Date.now() + product.name}`);
         const uploadTask = uploadBytesResumable(videoRef, product);
         ...
         uploadTask.on("state_changed", (snapshot) => {
           ...
           setAverageProgressVideos((prevProgress) => {
             // Create a new array with the updated progress for the specific index
     ...
     const docRef = await addDoc(collection(db, "videos"), {
       ...
       videos: videoUrls, // Store the download URLs in Firestore
     });
   } catch (error) {
            ...
            <form className="py-6 px-9">
              <div className="mb-6 pt-4">
                ...
                {newProductdata.videos.map((file, i) => {
                  console.log(AverageProgressVideos[i], "_)_)00000");
                  return (
                    <div
                      key={i + "lkkllklklklk"}
                    ...
                    >
                      ...
                      <div className="relative mt-5 h-[6px] w-full rounded-lg bg-[#E2E5EF]">
                        <div
                          style={{ width: `${AverageProgressVideos[i]}%` }}
